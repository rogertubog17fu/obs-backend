...
export const initApolloGraphqlServer = async (app: Express): Promise<ApolloServer> => {
  let userToken: string | string[] | undefined = '';
  ...
  const server = new ApolloServer({
    ...
    context: async ({ req, res, connection }) => {
      try {
        ...
        console.info('request as no header object');
      }
      ...
      const graphqlContext: IGraphQLContext = {
        ...
        // ability: await defineSystemAbilitiesFor(user?.id),
        ability: await defineSystemAbilitiesFor(),
      };
